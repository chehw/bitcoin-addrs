...
static void Transform(uint32_t* s, const unsigned char* chunk)
{
	...
	uint32_t w0 = le32toh(*(uint32_t *)&chunk[0]);
	uint32_t w1 = le32toh(*(uint32_t *)&chunk[4]);
	uint32_t w2 = le32toh(*(uint32_t *)&chunk[8]);
	uint32_t w3 = le32toh(*(uint32_t *)&chunk[12]);
	uint32_t w4 = le32toh(*(uint32_t *)&chunk[16]);
	uint32_t w5 = le32toh(*(uint32_t *)&chunk[20]);
	uint32_t w6 = le32toh(*(uint32_t *)&chunk[24]);
	uint32_t w7 = le32toh(*(uint32_t *)&chunk[28]);
	uint32_t w8 = le32toh(*(uint32_t *)&chunk[32]);
	uint32_t w9 = le32toh(*(uint32_t *)&chunk[36]);
	uint32_t w10 = le32toh(*(uint32_t *)&chunk[40]);
	uint32_t w11 = le32toh(*(uint32_t *)&chunk[44]);
	uint32_t w12 = le32toh(*(uint32_t *)&chunk[48]);
	uint32_t w13 = le32toh(*(uint32_t *)&chunk[52]);
	uint32_t w14 = le32toh(*(uint32_t *)&chunk[56]);
	uint32_t w15 = le32toh(*(uint32_t *)&chunk[60]);
...
void ripemd160_final(ripemd160_ctx_t * ripemd, unsigned char hash[20])
{
	...
	uint64_t num_bits = ((uint64_t)ripemd->bytes) << 3;
	*(uint64_t *)sizedesc = htole64(num_bits);
	
	ripemd160_update(ripemd, pad, 1 + ((119 - (ripemd->bytes % 64)) % 64));
	ripemd160_update(ripemd, sizedesc, 8);

	uint32_t * u32 = (uint32_t *)hash;
	for(int i = 0; i < 5; ++i) {
		u32[i] = s[i];
	}
	return;
